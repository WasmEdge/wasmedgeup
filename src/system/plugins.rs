...
///
/// Rules:
/// - macOS: darwin_<arch> (arm64 for AArch64, x86_64 otherwise)
/// - Windows: unsupported for plugin assets (return UnsupportedPlatform)
/// - Linux (glibc):
    ...
    match os.os_type {
        ...
        TargetOS::Linux | TargetOS::Ubuntu => {
            if matches!(os.libc.kind, LibcKind::Glibc) {
                let use_ml2014 = runtime_version <= &Version::new(0, 14, 999);
